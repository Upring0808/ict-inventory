...
export async function PATCH(request: Request) {
  try {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id) ||
      name.length < 2 || name.length > 100 || (username && !USERNAME_PATTERN.test(username)) ||
      (password && !validPassword(password))) {
      return Response.json({ error: 'Enter a valid name and username. A new password must be at least 12 characters.' }, { status: 400 });
    }
    ...
    if (password) {
      if (!account.auth_user_id) {
        return Response.json({ error: 'This account has no active sign-in identity. Recreate it before changing its password.' }, { status: 409 });
      }
      const { error } = await admin.auth.admin.updateUserById(account.auth_user_id, { password });
      if (error) return Response.json({ error: 'The password could not be updated.' }, { status: 400 });
    }
    ...
    const { error } = await admin.rpc('manage_authorized_account', {
      ...
      p_actor_email: actor.email,
      p_actor_name: actor.name,
      p_password_updated: Boolean(password),
    });
    if (error) {
      const conflict = error.code === '23505';
      return Response.json(
        { error: conflict ? 'That username is already in use.' : 'The authorized account could not be updated.' },
        { status: conflict ? 409 : 400 }
      );
    }
